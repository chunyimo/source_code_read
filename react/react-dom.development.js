 ...
 */
 ...
 'use strict';
  ...
  function markStarvedLanesAsExpired(root, currentTime) {
    ...
    while (lanes > 0) {
      // 将lanes转为32位无符号，从第一个1 计算，剩余的数字的长度
      // 如00000000000011110100001001000000 截取出 11110100001001000000， 长度为20.
      var index = pickArbitraryLaneIndex(lanes);
      var lane = 1 << index;
  ...
  // 函数返回一个数字在转换成 32 无符号整形数字的二进制形式后, 开头的 0 的个数,
  ...
  var UserBlockingPriority$1 = unstable_UserBlockingPriority;
      var runWithPriority = unstable_runWithPriority; // TODO: can we stop exporting these?
  ...
  function accumulateSinglePhaseListeners(targetFiber, reactName, nativeEventType, inCapturePhase, accumulateTargetOnly) {
    ...
    while (instance !== null) {
      var _instance2 = instance;
          ...
          var tag = _instance2.tag; // Handle listeners that are on HostComponents (i.e. <div>)
  ...
  function accumulateTwoPhaseListeners(targetFiber, reactName) {
    ...
    while (instance !== null) {
      var _instance3 = instance;
          ...
          var tag = _instance3.tag; // Handle listeners that are on HostComponents (i.e. <div>)
  ...
  function processUpdateQueue(workInProgress, props, instance, renderLanes) {
    ...
    // 本次更新前该fiber节点已保存的Update，firstBaseUpdate为链表表头，lastBaseUpdate为链表表尾
    var firstBaseUpdate = queue.firstBaseUpdate;
    var lastBaseUpdate = queue.lastBaseUpdate;

    // 触发更新时，产生的Update保存在shared.pending中形成单向环状链表。
    ...
    if (pendingQueue !== null) {
      ...
      // 在任何时候，一个组件实例最多有两个与之对应的Fiber对象：当前即将渲染的（current fiber）
    ...
    if (firstBaseUpdate !== null) {
      ...
      do {
        ...
        if (!isSubsetOfLanes(renderLanes, updateLane)) {
          ...
          if (newLastBaseUpdate === null) {
            ...
            // 先执行newLastBaseUpdate.next = clone， 在执行 newLastBaseUpdate = clone
      ...
      /*/*
      将剩余过期时间设置为队列中剩余的时间。
      这应该没有问题，因为影响过期时间的另外两个东西是props和context.
      当我们开始处理队列时，我们已经处于开始阶段的中间，所以我们已经处理了context。
      组件中的上下文指定了shouldComponentUpdate是很棘手的;但无论如何我们都要考虑这个。
      */
  ...
  function ChildReconciler(shouldTrackSideEffects) {
          ...
          case REACT_ELEMENT_TYPE:
            {
              if (newChild.key === key) {
                ...
                // 在updateElement 内部判断是否是同类型
                // 如果当前只有一颗fiber树（mount的情况），即oldFiber.alternate 为null，需要创建新的fiber，
                // 否则在olderFiber.alternate上做文章。
    ...
    function reconcileChildrenArray(returnFiber, currentFirstChild, newChildren, lanes) {
      ...
      // 第一次遍历，关键是比对key。在updateSlot中，如果key不一致，返回null，或者新节点为string|number，
      // 但旧节点有key，也返回null，然后跳出循环。
      for (; oldFiber !== null && newIdx < newChildren.length; newIdx++) {
        if (oldFiber.index > newIdx) {
          // 对应的是 [null, b] => [a, b]的情况，刚开始的时候只有b，b的index为1
          nextOldFiber = oldFiber;
          oldFiber = null;
        } else {
          // 正常情况下 为了下轮循环，拿到兄弟节点下面赋值给oldFiber。
        ...
        // 可能是复用，也可能是插入（newFiber.alternate === null 的情况）
    ...
    */
    function reconcileSingleElement(returnFiber, currentFirstChild, element, lanes) {
      ...
      while (child !== null) {
        ...
        // 单个节点，一般不设置key，都为null；也或者是由多个，经过删除后只剩下一个
        if (child.key === key) {
          // 判断类型是否相同
          switch (child.tag) {
            case Fragment:
          ...
          /*
          Note：注意这是单节点的情况，经过遍历后发现，有一个key相同（说明该fiber是对应的上一次更新的），
          但是类型又不同，那么其它兄弟也不会相同，即也不能复用，标记为删除。
          */
    ...
    // 使用协调的side-effect 来标记（tag）children。
    function reconcileChildFibers(returnFiber, currentFirstChild, newChild, lanes) {
      ...
      // note: 优化的点，使用fragment，直接取它的children来比较
  ...
  function useMutableSource(hook, source, getSnapshot, subscribe) {
    ...
    var _dispatcher$useState = dispatcher.useState(function () {
      return readFromUnsubcribedMutableSource(root, source, getSnapshot);
        ...
        var currentSnapshot = _dispatcher$useState[0];
        var setSnapshot = _dispatcher$useState[1];
  ...
  function rerenderDeferredValue(value) {
    var _rerenderState = rerenderState();
        var prevValue = _rerenderState[0];
        var setValue = _rerenderState[1];
  ...
  function mountTransition() {
    var _mountState2 = mountState(false);
        var isPending = _mountState2[0];
        var setPending = _mountState2[1]; // The `start` method can be stored on a ref, since `setPending`
    // never changes.
  ...
  function rerenderTransition() {
    var _rerenderState2 = rerenderState();
        var isPending = _rerenderState2[0];
      ...
      case Block:
        {
          ...
          if (updateQueue !== null) {
            ...
            if (lastEffect !== null) {
              ...
              do {
                var _effect2 = effect;
                    var destroy = _effect2.destroy;
                    var tag = _effect2.tag;
  ...
  function scheduleUpdateOnFiber(fiber, lane, eventTime) {
    ...
    // 如果fiber.lanes不为空，则说明该fiber节点有更新，而fiber.childLanes是判断当前子树是否有更新的重要依据，
    // 若有更新，则继续向下构建，否则直接复用已有的fiber树，就不往下循环了，可以屏蔽掉那些无需更新的fiber节点。
  ...
  function ensureRootIsScheduled(root, currentTime) {
    ...
    // 新任务优先级 拿到是root.pendingLanes中最紧急的那部分lanes对应的优先级。
    // 通过取最高优先级的方法，将来自同一事件中的多个更新收敛到一个任务中去执行。
  ...
  function completeUnitOfWork(unitOfWork) {
    ...
    do {
      ...
      if ((completedWork.flags & Incomplete) === NoFlags) {
        ...
        // 搜集在 processUpdateQueue 中跳过的lanes
        // 只收集当前正在complete的fiber节点的子节点和兄弟节点的lanes以及childLanes：
  ...
  function resetChildLanes(completedWork) {
    ...
    // completeUnitOfWork 是一个向上的过程，最终会被这些被跳过的任务被挂到root上
    // 在提交阶段，将childLanes放到root.pandingLanes
  ...
  function commitRootImpl(root, renderPriorityLevel) {
    ...
    if (firstEffect !== null) {
      ...
      // We do a separate pass of the effect list for each phase:
      // all mutation effects come before all layout effects, and so on.
      // The first phase a "before mutation" phase.
      // We use this phase to read the state of the host tree
      // right before we mutate it.
      // This is where getSnapshotBeforeUpdate is called.
      // 提交阶段被拆分为几个阶段。
      // 分开为每个阶段执行它的副作用。
      // 所有的mutation effects需要在layout effects之前。
      ...
      // layout, but class component lifecycles also fire here for legacy(遗留的) reasons.
