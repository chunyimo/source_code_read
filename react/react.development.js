 ...
 */
 ...
 'use strict';
  ...
  let assign = Object.assign || function (target, sources) {
    if (target == null) {
  ...
  function warnIfStringRefCannotBeAutoConverted(config) {
    {
      if (typeof config.ref === 'string' && ReactCurrentOwner.current && config.__self && ReactCurrentOwner.current.stateNode !== config.__self) {
        let componentName = getComponentName(ReactCurrentOwner.current.type);
  ...
  function cloneElement(element, config, children) {
    ...
    let props = assign({}, element.props); // Reserved names are extracted
  ...
  function mapIntoArray(children, array, escapedPrefix, nameSoFar, callback) {
    ...
    if (invokeCallback) {
      ...
      let childKey = nameSoFar === '' ? SEPARATOR + getElementKey(_child, 0) : nameSoFar;
    ...
    if (payload._status === Uninitialized) {
      ...
      thenable.then(function (moduleObject) {
        if (payload._status === Pending) {
          let defaultExport = moduleObject.default;
    ...
    if (!(dispatcher !== null)) {
      {
        ...
        This could happen for one of the following reasons:\n
          ...
          See https://reactjs.org/link/invalid-hook-call for tips 
  ...
  typeof MessageChannel !== 'function') {
    ...
    let performWorkUntilDeadline = function () {
      if (scheduledHostCallback !== null) {
        let currentTime = getCurrentTime(); // Yield after `yieldInterval` ms, regardless of where we are in the vsync
        ...
        try {
          let hasMoreWork = scheduledHostCallback(hasTimeRemaining, currentTime);
